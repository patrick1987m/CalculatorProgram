...
namespace ProgramTools
{
	...
	{
		// Implementation Found at /HeaderFiles/universalCalculatorTokenStreams.h
	}
	...
	{
		// Implementations of Point Struct:
		struct Point
		{
			double x;
			double y;
		...
		std::istream& operator>>(std::istream& is, Point& point)
		{
			...
			if (is >> ch1 >> point.x >> ch2 >> point.y >> ch3)
			{
				if (ch1 != '(' || ch2 != ',' || ch3 != ')')
				{
		...
		{
			double distanceFallen{ ProgramTools::UniversalScience::gravity * seconds * seconds / 2};
